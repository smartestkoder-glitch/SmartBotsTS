...
import autoDMconfig from "../../Bots settings/autoDMconfig.ts";
...
const autoDM = {
    ...
    waitLoadAllSlots: async (bot, lastSlot = 45, maxTime = 10000) => {
        let time = 0
        while (parseDM.decoder(bot).data.length !== 45) {
            await func.delay(30)
            time += 30
            if (time > maxTime) {
    ...
    openDM: async (bot) => {
        bot.chat("/dm")
        if (!await window.waitToSlot(bot, 21, "gold_ingot", 10000)) return restart.default(bot, "Не открылся dm")
        window.click(bot, 21)

        if (!await window.waitToSlot(bot, 49, "nether_star", 10000)) return restart.default(bot, "Не открылся dm")
    },
    ...
    buyLot: async (bot, slot) => {
        ...
        if (!await window.waitToSlot(bot, 0, "lime_stained_glass_pane", 5000)) return
    ...
    autoDMChecker: async (bot, buyCourse) => {
        ...
        while (bot.currentWindow) {
            ...
            if (normOffer) {
                func.output("Пытаюсь купить лот...", "dev", "green", "old")
