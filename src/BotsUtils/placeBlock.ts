...
export default (bot :Bot, block :Block, face : 1 | 2 | 3 | 4 | 5 | 6, item :string, hand : 0 | 1, cx :number = 0.5, cy :number = 1, cz :number = 0.5) => {
    ...
    try {
        ...
        if (bot.heldItem?.name !== item && hand === 0) return
        if (bot.inventory.slots[45]?.name !== item && hand === 1) return
        ...
        if (hand === 0) bot.swingArm("right")
        if (hand === 1) bot.swingArm("left")
