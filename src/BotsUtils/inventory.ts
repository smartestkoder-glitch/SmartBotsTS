...
const inventory = {
    ...
    restackItemShift: async (bot :Bot, item :string, fslot :number = 9, lslot :number = 35, kolvo :number = 0, maxsize :number = 64) => {
     ...
     * @param count
     */
    getSlotItem: (slots :(Item | null)[], item :string, fslot :number = 9, lslot :number = 45, count :number = 1) => {
     ...
     * @param lslot - Последний слот, на котором заканчивать искать
     */
    getAllSlotsItem: (slots :(Item | null)[], item :string, fslot :number = 9, lslot :number = 45) => {
        if (!slots) return func.output("Спас от ошибки, саня молодец")
     ...
     * @param lslot - Последний слот, на котором заканчивать искать
     */
    getSlotEmpty: (slots :(Item | null)[], fslot :number = 9, lslot :number = 45) => {
        if (!slots) return func.output("Спас от ошибки, саня молодец")
     ...
     * @param lslot - Последний слот, на котором заканчивать искать
     */
    getCountItem: (slots :(Item | null)[] | undefined, item :string, fslot :number = 9, lslot :number = 45) => {
        if (!slots) {
    ...
    getCountEmptySlot: (slots :(Item | null)[] | undefined, fslot :number = 9, lslot :number = 45) => {
        if (!slots) return func.output("Спас от ошибки, саня молодец")
     ...
     * @param slots
     */
    changeInvToNew: (slots :(Item | null)[]) => {
        if (!slots) return func.output("Спас от ошибки, саня молодец")
     ...
     * @param item
     */
    equipHand: async (bot :Bot, item :Item) => {
        if (!bot.smart.vars.work) return
        try {
