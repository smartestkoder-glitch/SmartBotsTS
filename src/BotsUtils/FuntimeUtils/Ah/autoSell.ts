...
const autoSell = {
     ...
     * @param newVersion
     */
    sellOneItem: async (bot :Bot, item :string, price :number, newVersion = false) => {
        ...
        if (!bot.smart?.vars?.script?.autoSell?.boughtCountItem) return
        ...
        if (!needItem) return autoSell.updateNeedToBuy(bot, true)
    ...
    sellOneInvisOld: async (bot :Bot, priceSell :number) => {
     ...
     * @param newVersion
     */
    resell: async (bot :Bot, newVersion :boolean = false) => {
        ...
        if (typeof boughtCountItem !== "number") return
