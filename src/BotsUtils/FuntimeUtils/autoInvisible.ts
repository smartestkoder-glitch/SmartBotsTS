...
const autoInvisible = {
     ...
     */
    transferAndDrinkInvis: async (bot :Bot) => {
        if (!bot.smart.vars.work) return
    ...
    drinkIfTime: async (bot :Bot, time :number) => {
        await func.delay(100)

        const effect = autoInvisible.checkEffect(bot)
        if (!effect?.duration || effect.duration < time) return await autoInvisible.transferAndDrinkInvis(bot)
