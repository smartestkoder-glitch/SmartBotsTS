    ...
    moneyUpdateEvent: (bot) => {
        bot._client.on("teams", (packet) => {
            ...
            if (money === undefined || isNaN(money))
                return;
            if (bot.smart.vars.money?.balance === undefined || isNaN(bot.smart.vars.money?.balance))
                return console.log("Хуня!");
            bot.smart.vars.money.balance = money;
            if (bot.smart.vars.money.autoClan)
                bot.chat("/clan invest " + money.toString());
        });
    },
    getMoney: (bot) => {
